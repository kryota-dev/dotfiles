...
//   値だけ作ると「効いていない設定を効いているつもりで記録する」ことになるため、
//   policy に network の軸を置かない（許可の実装は #535 / #502）。
//   なお **provider ごとにネットワークの実効性は異なる**: Claude は設定 JSON の
//   strict allowlist、Codex は workspace-write の既定 off（いずれも #526 で実測）である一方、
//   **Antigravity のネットワーク既定は #526 §5 で「未確認」のままで、adapter も制御する
//   argv を出さない**。したがって「全 provider でネットワークが閉じている」とは主張しない。
