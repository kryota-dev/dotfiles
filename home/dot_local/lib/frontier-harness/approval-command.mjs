...
// approval-rules-baseline.mjs との同期は tests/frontier_harness_approval.test.mjs が
// 機械検査する（`<binary> <subcommand>` 形のルールが名指す binary は全てここに載る）。
//
// この基準により `make` / `go` / `pytest` は**入れない**。manifest が承認しうる runner
// （manifest-policy.mjs の `APPROVABLE_*`）ではあるが baseline ルールが名指ししないので、
// 足しても見逃しうるルールは増えないまま `make -C "$DIR" lint` / `pytest -k "$PAT"` が
// 同期問い合わせに変わるだけになる。承認済みコマンドとの照合を fail-closed に保つ役目は、
// この集合ではなく `analyzeShellCommand` が返す `dynamic` が担う（分業の理由は同関数の doc）。
...
function skipGlobalOptions(binary, tokens) {
  ...
  //
  // **ここでトークンの dynamic を検査しないのは意図的である**（分業の理由は
  // `analyzeShellCommand` の doc。要点は「ここを倒すと `cat "$TMPDIR/x"` のような日常操作
  // まで escalation 側の同期問い合わせに変わる」「allowlist 側の fail-closed は別に返す
  // `dynamic` が担保する」の 2 つ）。`commandSegments` / `matchCommand` の一致判定を触る
  // ときは、この分業を壊していないかを確かめること。
...
// （この分業の説明はここが canonical。`SUBCOMMAND_DISPATCHED_BINARIES` と
// `skipGlobalOptions` のコメントはここを参照する側なので、直すならまずここを直すこと。）
//
