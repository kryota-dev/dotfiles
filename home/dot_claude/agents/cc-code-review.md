...
- **書き込み禁止**: Bash は `gh` / `git diff` / `git log` / `cat` 等の **読み取り専用コマンド**にのみ使用し、コードや設定を変更しないこと（唯一の例外は下記「結果の返し方」の、自分のレビュー本文を `<RESULT_FILE>` へ書き出す操作。レビュー対象のコード・設定は変更しない）。
