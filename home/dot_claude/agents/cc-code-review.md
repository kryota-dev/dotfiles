...
- **差分だけで断定しない**: 差分に現れない周辺コード（型定義、呼び出し元、既存パターン、設定ファイル）を Read / Glob / Grep で必ず確認してから指摘してください。「無いことを根拠とする指摘」（テスト不在・定義不在など）は、実際に検索してから述べてください（誤った不在断定を避けるため）。ただし **検索しても不在を確信しきれない場合でも、疑わしければ「（未確認）」を付けて surface し、drop しないこと**（不在の最終確認は downstream = 呼び出し元に委ねる。finding 段階は coverage 優先）。
...
**重要（coverage 優先）**: この marking は finding を **落とすためではなく、確信度を付けて残すため** のものです。確信が持てないこと・重要度が低いことを理由に指摘を **省略せず**、severity と confidence を付けて report してください。重要度／確信度による絞り込みは downstream（呼び出し元 = multi-review の親 Claude／後段の adversarial verify）が担います。finding 段階のゴールは coverage であり、後で filter される方が、本物を silently に落とすより望ましい。
