...
- Phase 0.5（既存 auto-memory の再検証）は \`python3 ~/.agents/skills/knowledge-distill/scripts/memory-revalidate.py --format text\` を**単独のコマンド**として実行し、出力をレポートへ 1 節としてそのまま転記してください（env prefix やパイプを付けると許可リストの prefix に一致せず拒否されます）。instinct 蓄積が閾値未満で縮退終了する場合も、この節だけは省略しないでください。検出結果は報告のみとし、memory は変更しないでください。
