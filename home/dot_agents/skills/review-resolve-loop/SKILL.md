...
### 1-1b. 未返信の review body を取得（body 単独レビューに限定）
...
**重要**: review body には 2 種類ある:

1. **個別 thread のサマリー** （`reviews/{reviewId}/comments` に thread comments が紐づく）: 例 `"Actionable comments posted: 2"`、`"1 new potential issue"`。本体は thread 側にあるため、review body 自体への返信は不要（個別 thread で対応する）
2. **body 単独レビュー** （thread comments が紐づかない）: 例 CoderabbitのNitpick が body 内に inline コードや diff として書かれているケース。thread では返信できないため、review body への返信が必要になる

スキルは **(2) body 単独レビューのみ** を処理対象とする。(1) サマリーは個別 thread の処理で完結するため、Phase 1-1b の取得段階で除外する。

```bash
MY_LOGIN=$(gh api user --jq .login)
TMP_REVIEWS=$(mktemp)
TMP_BODY_ONLY=$(mktemp)
TMP_REPLIED=$(mktemp)
...
# Step 1b: 各 review に紐づく thread comments の有無を判定（body 単独レビューのみを残す）
echo "[]" > "$TMP_BODY_ONLY"
for review_id in $(jq -r '.[].reviewId' "$TMP_REVIEWS"); do
  thread_count=$(gh api "repos/{owner}/{repo}/pulls/{PR番号}/reviews/${review_id}/comments" --jq 'length')
  if [ "$thread_count" -eq 0 ]; then
    # 個別 thread に紐づかない = body 単独レビュー（返信対象候補）
    jq --argjson id "$review_id" '. + [$id]' "$TMP_BODY_ONLY" > "${TMP_BODY_ONLY}.new" && mv "${TMP_BODY_ONLY}.new" "$TMP_BODY_ONLY"
  fi
done
...
# Step 3: 「body 単独レビュー」かつ「未返信」の review body のみフィルタ
jq --slurpfile bodyOnly "$TMP_BODY_ONLY" --slurpfile replied "$TMP_REPLIED" '
  [.[]
   | select((.reviewId | tostring) as $rid | ($bodyOnly[0] | map(tostring) | index($rid)))
   | select((.reviewId | tostring) as $rid | ($replied[0] | map(tostring) | index($rid)) | not)
  ]
' "$TMP_REVIEWS"

rm -f "$TMP_REVIEWS" "$TMP_BODY_ONLY" "$TMP_REPLIED"
```

**設計意図**:
- review body は制御文字を含む場合があるため、シェル変数ではなく一時ファイル経由で処理する
- hidden marker 方式により、スキル再実行時の重複処理を防止する
- **body 単独レビュー (thread に紐づかない) のみを処理対象とする**: 個別 thread のサマリー review body に過剰返信する問題を防ぐ。サマリーは個別 thread で対応すれば十分
...
Phase 1-1b で取得した review body （body 単独レビュー）の本文を読んで、以下のいずれかに分類:

| パターン | 例 | 対応方針 |
|---|---|---|
| **body 自体が実質的な指摘** | CoderabbitのNitpickコメントが body 内に inline コードや diff として書かれているケース | **返信対象**（thread では返信できないため、Phase 4 で Issue comment として返信） |
| **No Issues / 内容なしの ack** | devin "No Issues Found"、人間の "見ました！！" | **返信不要**（指摘なし、ノイズ回避。ボット・人間問わず適用） |

**判定基準**: body 内に「修正提案」「具体的な懸念」「コード差分」等の実質的な指摘があるかどうか。挨拶・要約・「No Issues」のみなら返信不要。
...
**review body への返信**（body 単独レビューに限定）:

review body はスレッドではないため、Issue comment として投稿する。返信済み判定用の hidden marker を必ず含める。

**ただし、Phase 2-3b で「返信対象」と判定された body 単独レビューに対してのみ実施する**。個別 thread のサマリー review body や No Issues / ack には返信しない（重複応答とノイズを回避）。
...
```bash
# 返信本文は @ メンションを含むため一時ファイル経由で投稿する（4-3 参照）
gh api repos/{owner}/{repo}/issues/{PR番号}/comments \
  --method POST \
...
**全レビュアー（人間・ボット問わず）への返信**: 冒頭に `@{reviewer}` メンションを付ける。`coderabbitai[bot]` のように `[bot]` サフィックスを持つレビュアーは、メンションから `[bot]` を除いた `@coderabbitai` 形式にする（`coderabbitai` 等は `@` 言及で再レビュー等に反応する）。CI レビューコメント（`github-actions[bot]`）への返信も同様に `@github-actions` を付ける。
...
> **Note**: 人間・ボットいずれの返信にも `@{reviewer}` メンションを付ける。`[bot]` サフィックスはメンションから除く（`coderabbitai[bot]` → `@coderabbitai`）。`github-actions[bot]` 宛のメンション（`@github-actions`）は通知効果を持たないが、返信の対象を明示するため一貫して付与する。
...
| 個別 thread のサマリー review body | Phase 1-1b の `hasThreadComments` フィルタで取得段階から除外。返信不要（個別 thread で対応） |
