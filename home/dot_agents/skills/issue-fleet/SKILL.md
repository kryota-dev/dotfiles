...
**共有作業ツリーへの書き込みガードは `pr-workflow`「共有作業ツリーでの Claude subagent 委譲契約」節が唯一の SSOT**。ここに再掲せず必ずそれに従い、下記テンプレートに規約を含める（直列レーンは同一 worktree を複数 Issue で使い回すため、前のエージェントの成果を旧版へ巻き戻す事故が起こりうる）。
