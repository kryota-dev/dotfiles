...
## Frontier Harness への段階移行

`frontier-harness` の新規 path は `/execution-readiness-check` を使い、adapter capability、
account scope、rollout、permission manifest を確認する。shadow rollout の間は、本 skill を
`wave-orchestrator` を含む legacy caller の安全 gate として維持する。

全 legacy caller が移行し、pilot telemetry が十分に蓄積されるまで、この §4 contract と
over-provision gate を削除・弱体化してはならない。移行完了後に本 skill を compatibility shim へ
縮退するかを別 PR で判断する。
