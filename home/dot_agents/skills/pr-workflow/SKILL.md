...
## turn を跨いで待たない（実行モデル契約）

**turn を終えてよいのは、`AskUserQuestion` で user の判断を待つときだけである。** それ以外の理由で turn を終えない —— とりわけ**バックグラウンドの結果待ちで終えない**。「完了通知が来次第 Phase 6 へ進めます」と述べて turn を終えると、`claude -p`（`fh session launch` の子）には**次の turn が無いのでその通知は永久に来ず**、Phase 5〜7 が丸ごと実行されない。しかもターンはエラー無く終わるので `succeeded` / `exitCode: 0` として記録される（#585 / #573）。
...
| 待つ対象 | join の形 |
|---|---|
| CI（Phase 5） | `/monitor-ci` の `gh pr checks --watch` を**前景で**実行する。`run_in_background: true` へ回さない |
| サブエージェント（Phase 1-4 の worker、Phase 5 のログ triage） | `Agent` を **`run_in_background: false`** で起こす（既定は background で、完了は次 turn の通知として届く） |
| 名前付き teammate として背景に起こすサブエージェント（Phase 6 の `adversarial-verifier`） | 背景起動のままでよいが、**同じ turn の中で `<RESULT_FILE>` の出現をポーリングして join する**（完了報告の通知を待って turn を終えない） |
| バックグラウンドの Bash（`/multi-review` の Codex leg 等） | 起動した turn の中で、結果ファイルの出現を**前景でポーリング**して join する |

```bash
# <RESULT_FILE> が出るまで前景で待つ。上限は必ず有限にする。
deadline=$((SECONDS + 540))
until [ -s "<RESULT_FILE>" ]; do
  if [ "$SECONDS" -ge "$deadline" ]; then echo "WAIT-INCOMPLETE"; exit 0; fi
  sleep 15
done
echo "WAIT-DONE"
```
...
**CI の完了を待つのはこの turn の中である**（「turn を跨いで待たない」節）。`gh pr checks --watch` を背景へ回して turn を終えない。ログ triage の worker も `run_in_background: false` で起こす。
...
| GATE 3 | Phase 7 `/review-resolve-loop` 完了後（**到達点検査を通してから**） | **merge-ready handoff** | **user 承認待ち**（merge は常に user） |
...
- **turn を跨いで待たない**: バックグラウンドの結果待ちで turn を終えない。待機は tool call の繰り返しで turn 内に完結させ、GATE 3 の直前に到達点を PR の外形から検査する（「turn を跨いで待たない（実行モデル契約）」節）。
