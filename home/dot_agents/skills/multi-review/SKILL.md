...
2. **同一メッセージ内で 常設 3 ツール + マッチ specialist を並列起動**:
...
**finding 段階 = coverage / 親 = downstream filter（#224）**: reviewer 定義（各エージェントの system prompt）は Opus 4.8 の推奨に沿い、**finding 段階では coverage を優先**し、確信が持てない・重要度が低い指摘も severity/confidence（「（未確認）」等）を付けて surface するよう指示されている。したがって **重要度・確信度による絞り込み（filter）は親 Claude のこの検証段階（+ pr-workflow の adversarial verify）が担う downstream の責務** である。親は「（未確認）」marking を無検証で drop せず、下表の手段で裏取りしたうえで採否を判断する（本物の bug を silently に落とさない = recall を保つ）。
