...
argument-hint: "<PR番号 | owner/repo#PR番号 | PR URL> [--arch]"
---
...
以下の優先順で判定し、**owner/repo と PR 番号の 2 つ組**を確定する。以降の全 `gh` 呼び出しには `--repo <owner/repo>` を明示すること（cross-repo バッチで cwd の repo に暗黙解決される事故を防ぐ）:

1. **PR URL** (`github.com/<owner>/<repo>/pull/<N>` を含む URL): 正規表現でパスから `owner`, `repo`, `N` を抽出
2. **`owner/repo#PR番号`** (`^[\w.-]+/[\w.-]+#\d+$`): `#` 前で owner/repo、後ろで PR 番号を分割
3. **PR番号のみ** (`^\d+$` または `^#\d+$`): 現在の作業ディレクトリの owner/repo を `gh repo view --json nameWithOwner --jq '.nameWithOwner'` で解決し、それを補う
4. **引数なし**: 現在のブランチに関連する PR を `gh pr view --json number,url --jq '.number'` で自動検出。owner/repo は同じく cwd から `gh repo view` で補う
5. **`--arch`**（任意フラグ）: 指定時のみ **aggregate-view reviewer**（`architecture-reviewer`）を別レイヤで追加 spawn する（「aggregate-view reviewer」節参照）。未指定時は spawn しない（毎 PR は走らせないコスト方針）。pr-workflow の large tier からは自動でこのフラグ相当が要請される。
...
3. Phase 1 手順 1 で確定した owner/repo をそのまま Phase 1.5 以降の `gh api repos/{owner}/{repo}/…` プレースホルダに差し込む（追加の `gh repo view` は不要）
...
レビュー（body サマリー・インラインコメント）に AI ツールのクレジット・署名（`Co-Authored-By` / `Generated with ...` 等）を付与しない（`~/AGENTS.md` の global ルール準拠）。
