...
- **検出方法**: Phase 2 開始前に `gh pr diff <番号> --name-only`（または取得済み差分のファイル一覧）で変更ファイルの拡張子・パスを取得し、上表にマッチする specialist を選ぶ。
...
- 動的 specialist は **マッチした言語のみ** spawn し、`model: sonnet`（エージェント frontmatter 固定）で動くため、常設ツールより安価かつ的を絞った追加コストに留まる。非対象言語の PR では追加コストゼロ
