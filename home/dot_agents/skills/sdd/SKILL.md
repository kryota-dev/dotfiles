...
考察した構成に基づき、各レビューエージェントを **Agent ツール** でスポーン。事前のチーム作成は不要（Agent ツールを直接呼び出すだけでよく、スポーンしたエージェントはセッション終了時に自動クリーンアップされる）。
...
### 5-5. 全レビューエージェント解決の確認

全レビューエージェントが APPROVE（またはシャットダウン済み）になるまで 5-4 を繰り返す。全エージェントのシャットダウン完了後、チームの明示的な削除は不要（スポーンしたエージェントはセッション終了時に自動クリーンアップされる）。
