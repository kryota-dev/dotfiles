...
gh pr view <番号> --repo <owner/repo> \
  --json number,title,url,repository,isDraft,author,mergeable,labels

# PR 番号のみの場合: cwd の repo を暗黙で使う（--repo 省略可）。
# 明示指定時の draft は除外しない（下記例外参照）
```
...
gh search prs --review-requested=@me --state=open --limit ${LIMIT:-30} \
  --json number,title,url,repository,updatedAt,author,isDraft,mergeable,labels
```

- **`mergeable` と `labels`** は Phase 2 の分類（マージコンフリクト / WIP ラベル / draft 判定）で必須のため、必ず `--json` に含める（欠けると Phase 2 の判定が成立しない）。
- **draft PR は除外**する（`isDraft == true`）。**ただし引数で明示指定された draft PR は除外しない**（明示 = user が意図的に対象化していると解釈）。明示指定 draft は Phase 3 の計画表で `[draft]` タグを付けて可視化し、user が計画表で改めて対象化するか判断できるようにする（silent drop 禁止）。
...
| **C（除外）** | `mergeable != "MERGEABLE"`（コンフリクト等）、WIP マーカー（タイトルの `[WIP]` / `WIP:`、または `Draft` ラベル）あり、または diff 取得不可。**分類が不確実な場合もここに倒す** |

- 分類根拠（`mergeable` / `labels` / diff サイズ の値と、それが A/B/C 判定にどう作用したか）は Phase 3 の計画表に一言添える。
- **draft の扱い**: 引数で明示指定された draft PR は C に倒さず、通常の A/B 判定を行う（`[draft]` タグは計画表に付ける）。自動収集分の draft は Phase 1 の時点で除外済みのためここには来ない。
