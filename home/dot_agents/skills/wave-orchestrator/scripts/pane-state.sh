...
[ -n "$PREFIX" ] || { echo "--prefix は必須" >&2; exit 2; }
command -v tmux >/dev/null 2>&1 || { echo "tmux が無い" >&2; exit 3; }
...
if [ "$SELF_CHECK" = "1" ]; then
  ...
  [ -n "$PANES" ] || { echo "SELF-CHECK FAIL: prefix '$PREFIX' に一致する pane が無い" >&2; exit 4; }
  p=$(printf '%s\n' "$PANES" | head -1 | cut -d'|' -f2)
  cap=$(tmux capture-pane -p -t "$p" -S -$CAPTURE_LINES 2>/dev/null)
  [ -n "$cap" ] || { echo "SELF-CHECK FAIL: pane $p のキャプチャが空" >&2; exit 4; }
...
printf '%s\n' "$PANES" | while IFS='|' read -r w p t; do
  ...
  if   [ -z "$cap" ];                                          then st=GONE
  elif printf '%s' "$cap" | grep -qE "$MARK_BGWAIT";            then st=BUSY
  elif ! cmp -s "$D/$p.a" "$D/$p.b";                            then st=BUSY
  elif printf '%s' "$cap" | grep -q "$MARK_ENDED";              then st=ENDED
  elif printf '%s' "$cap" | grep -q "$MARK_SELECTOR";           then st=ASK
  else                                                               st=IDLE
  fi
  printf '%s %s %s\n' "$p" "$name" "$st"
done
