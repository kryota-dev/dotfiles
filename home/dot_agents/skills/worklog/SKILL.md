...
daily-planning skill の Step 1 と同じ考え方で、JST 基準の期間と gh search 用の日付レンジを算出する:
...
- gh search の日付フィルタは **UTC 解釈**のため、JST 期間を正確に切るには前後 1 日広めに取得し、返却された `closedAt`/`createdAt`（UTC）を JST に変換して絞り込む（daily-planning の START_UTC/END_UTC 方式）。
...
gh search prs --author=@me --merged-at "${START_DATE}..${END_DATE}" --limit 100 \
  --json repository,number,title,url,closedAt

# 2. 期間内に作成した PR（進行中の仕事）
gh search prs --author=@me --created "${START_DATE}..${END_DATE}" --limit 100 \
...
# 4. 期間内に自分が関わった Issue
gh search issues --involves=@me --updated "${START_DATE}..${END_DATE}" --limit 100 \
  --json repository,number,title,url,state,updatedAt
```

ローカルソース:

- **session-summary**: `ghq list -p` の各 repo で `<repo>/.kryota-dev/claude/session-summary/*.md` を列挙し、ファイル名先頭 timestamp（`YYYYMMDDHHMMSS`）が期間内のものを Read する（`/worktrees/` 配下のものは親 repo に集約して扱う）。
...
`--csv` 時は同名 `.csv` も出力する。ヘッダ: `date,repo,type,ref,title,url`（`type` は `pr-merged|pr-opened|review|issue|commit|session`、`date` は JST の `YYYY-MM-DD`）。
