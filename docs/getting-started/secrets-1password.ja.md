...
4. 以下に示す 4 つの Vault アイテムすべてが `kryota.dev` Vault に存在すること。
...
|-----|---|
| Vault | `kryota.dev` |
| アイテムタイトル | `Dotfiles - OpenRouter API` |
| フィールド参照 | `credential` |
| op:// URI | `op://kryota.dev/Dotfiles - OpenRouter API/credential` |
| レンダリング先 | `~/.config/zsh/dmux-secrets.zsh`（`private_dmux-secrets.zsh.tmpl`） |
| ファイルモード | `0600` |

dmux の AI 機能（スマートブランチスラッグ、AI コミットメッセージ、ペーン状態分析、`aiMerge`）が使用します。`OPENROUTER_API_KEY` を設定します。
...
| `Dotfiles - OpenRouter API` | 検証ゲートで `chezmoi apply` が exit 1 | `dmux-secrets.zsh` がレンダリングされない、dmux AI 機能が使用不可 |
...
- レンダリングされた `.zsh` ファイルは未エクスポートの変数（`export` なし）を使用するため、インタラクティブシェルのすべての子プロセスに値がリークしません。`claude.zsh` と `dmux.zsh` のランチャー関数がそれぞれのサブプロセスのみにスコープして再エクスポートします。
