...
| `lint-deno` | ntfy dashboard（kryota-dev/dotfiles#371）に対する `deno check` + `deno fmt --check`。best-effort で `deno` が無ければスキップ。`lint`/CI には含まれない（Darwin 限定機能で CI に Deno インストールなし） |
