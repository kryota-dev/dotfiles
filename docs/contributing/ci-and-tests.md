...
- **`--profile main` injection.** Injected only when argv carries no `--profile`/`-p` flag (all forms: `--profile x`, `--profile=x`, `-p x`, `-px`); a `--profile` substring inside a prompt argument does not falsely suppress injection; the scan stops at a literal `--`; the flag is always inserted ahead of any subcommand (`exec`, `exec review`).
