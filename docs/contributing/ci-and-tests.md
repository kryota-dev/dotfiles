...
Behavioural tests for `make lint-console`, the guard that replaced the `stop:check-console-log` hook retired in #520 (#522). The target's `CONSOLE_LINT_ROOTS` variable is overridden to point the scan at a fixture tree in `BATS_TEST_TMPDIR`, so the failing cases can be asserted without planting a violation in the repo. Covered: a bare `console.log` fails; `console.error` / `console.warn` fail too (the rule is `console.*`, not `console.log` alone); a `// deno-lint-ignore no-console` comment exempts its own line but not a second call in the same file; `process.stdout.write` passes; `console.log` inside a string literal passes (the check is AST-based, not textual); `.ts` and plain `.js` sources are both scanned, at any depth; an empty tree fails rather than silently passing; the repository itself passes; the target is wired into `make test`; and both CI jobs resolve deno from the mise pin without repeating the version.
