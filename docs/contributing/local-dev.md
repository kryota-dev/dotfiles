...
| `lint-deno` | `deno check` + `deno fmt --check` on the ntfy dashboard (kryota-dev/dotfiles#371); best-effort, skips if `deno` is absent. Not part of `lint`/CI — Darwin-only feature, no Deno install in CI |
