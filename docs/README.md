...
| [Notification channel: self-hosted ntfy over Tailscale](architecture/notifications.md) | The containerised ntfy server, `tailscale serve` ingress, the hook's event→priority map and fallback behaviour, and the one-time bootstrap |
...
| [Account isolation: aliases & env](agents/account-isolation.md) | Per-account env var table, full alias matrix, and `_claude_with_home` |
