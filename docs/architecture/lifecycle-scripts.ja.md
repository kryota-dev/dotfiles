...
ハードゲートです。`op` がインストール済みかつ認証済みであることを確認し、4つの必須 vault 参照に対して `op read` を呼び出します。
...
いずれかが失敗すると非 0 で終了し、after フェーズを中断します。このアイテムリストは `claude-secrets.zsh`、`dmux-secrets.zsh`、AWS config テンプレートが実際に使用するものと常に同期させる必要があります。
...
1. 順序付きタイムラインに自然に収まるプレフィックスを選ぶ。現在の空きスロット: `…15…17…19…30…`（40 より前）、`…41-49…`（sheldon とログインシェルの間）。
