...
ハードゲートです。`op` がインストール済みかつ認証済みであることを確認し、3つの必須 vault 参照に対して `op read` を呼び出します。
...
いずれかが失敗すると非 0 で終了し、after フェーズを中断します。このアイテムリストは `claude-secrets.zsh` と AWS config テンプレートが実際に使用するものと常に同期させる必要があります。
