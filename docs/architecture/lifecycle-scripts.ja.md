...
- **Xcode ライセンス**。developer directory が選択されていてライセンスが未同意の場合、Homebrew は `You have not agreed to the Xcode license` で**一切動作しません**。つまり未同意のライセンスは、10 が何かをインストールする前にそれを落とします。判定は Homebrew 自身のゲート（`Library/Homebrew/brew.sh`）を両方の分岐ごと写し取っています。すなわち (1) developer directory が `/` の場合は報告するだけで触らない（brew はこの状態を明確に拒否します。`xcrun` が**ハング**するためで、ハングした before スクリプトは失敗した場合と同じくらい確実に apply を止めます）、(2) それ以外で「`xcrun --find clang` が非ゼロ」かつ「その出力が license に言及している」場合に同意処理を走らせる、の 2 つです。いずれも「`xcode-select -p` が非空」を前提とします。brew と同一のテストを使うことで、brew が実際に拒否するときだけ動作し、それ以外のマシン（Command Line Tools のみの環境を含む）では `sudo` を一切要求しません。真っ新な Mac では本質的に 2 フェーズになります。Xcode.app 自体が 10 の `mas "Xcode"` で入るため、ライセンスの関門は**次回**の apply でしか現れないからです。
...
**失敗ポリシー**。`brew bundle` は**いずれか 1 件**でも失敗すると非ゼロで終了します。これは `before_` スクリプトなので、その status をそのまま伝播させると dotfiles が 1 つも書かれないまま apply 全体が中断します。引退した App Store アプリや Intel 専用 cask のせいでシェル設定を失うのは割に合わないため、部分失敗ではスキップされた対象を列挙した囲み警告を出したうえで exit 0 します。成功行 `Brew bundle complete.` はクリーンな実行時のみ出力されます。exit code だけでは「そもそも実行できなかった」と区別できない（Brewfile が無い場合も exit 1）ため、致命的な 2 条件（使える Homebrew が無い / Brewfile が読めない）は明示的に事前検査し、従来どおり `exit 1` します。Homebrew の事前検査は `command -v brew` ではなく、実体を絶対パスで解決して `--version` を実行する形にしています。`~/.local/launchers` が `PATH` の先頭に置かれており（[シェル環境](shell-environment.ja.md) 参照）、そこにある `brew` shim は real brew が存在しないとき 127 で終了します。`command -v` はその shim を受け入れてしまい、部分失敗の経路が 127 を「何もインストールしていないのに成功した apply」へ正規化してしまうためです。この挙動は macOS と Linux で同一です。CI は影響を受けません。`setup-validation.yml` はこのスクリプトを退避したうえで、フィルタ済み Brewfile に対して独自の厳格な `brew bundle` を実行するため、Brewfile の破損はそちらで検出され続けます。
