...
| `executable_` | デスティネーションをモード `0755` で作成 | `executable_statusline.sh` → `~/.local/bin/statusline.sh` (0755) |
...
| `run_once_` プレフィックス | スクリプトのコンテンツ SHA256 をキーとして一度だけ実行 | `run_once_after_40-setup-sheldon.sh.tmpl` |
...
| `.ecc.skills` | string 配列 | 採用済み ECC スキル名の 127 エントリリスト。`.chezmoiexternal.toml` でレンジされ、スキルごとに 1 つの external エントリを生成 |
...
| フラグメント | インクルード元 | 目的 |
|-------------|--------------|------|
| `coding-standards.md` | `AGENTS.md.tmpl`、`dot_claude/CLAUDE.md` | ハウスコーディング標準（日本語）。一度作成して両ハーネスのルールファイルに埋め込む |
