...
現在のエントリは、時間をかけて追加されてきた複数の無関係な廃止の波にまたがります: 廃棄された `sdd-*` エージェントファイル、実行時に CLI がサービスするようになった `agent-browser` の専用スキルとディスカバリースタブ自身の凍結された `references/`・`templates/` コピー、廃止された dmux プロビジョニングレイヤー、2026-07-06 のキュレーテッドインベントリ棚卸しで削除されたスキル群、そして `github-projects/skill.md` のケースリネームに伴うクリーンアップです。正確な最新の一覧は `home/.chezmoiremove` 自体を参照してください — この段落が実体とずれないよう、件数は意図的にここへ繰り返していません。
