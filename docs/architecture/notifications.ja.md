...
- **予算** (#643): `--max-turns` は
  <!-- FACT:knowledge-distill-max-turns -->80<!-- /FACT -->、watchdog は
  <!-- FACT:knowledge-distill-timeout-seconds -->1200<!-- /FACT --> 秒で発火します。
  どちらも「症状が止まるまで上げた」値ではなく実測から逆算した値です。セッション
  トランスクリプトが残っていた 3 回の失敗実行では、スループットは tool 呼び出し 1 回あたり
  10.3〜10.8 秒で一致していました。`--max-turns` で死んだ 2 回は 523 秒 / 539 秒でターン 50 に
  到達し、作業は未完でした。呼び出しを束ねた 1 回（1.93 呼び出し/ターン）は 597 秒で 58 回の
  呼び出しをこなし、代わりに watchdog に殺されました。そして唯一の成功は 600 秒の予算のうち
  589 秒を使っており、余裕は 1.8% でした。つまり **50 ターンと 600 秒は同じ壁**です ——
  直列化した 50 ターンは 600 秒をほぼ使い切るため、`--max-turns` の**上に載せる**ガードとして
  文書化されている watchdog が実質的に主たる上限になっており、どちらが先に発火するかは
  「その週にエージェントが呼び出しを束ねたかどうか」だけで決まっていました。80 ターン ×
  約 10.5 秒は約 840 秒で 1200 秒の内側に収まるため、`--max-turns` が再びコスト上限になります。
  修正のもう半分はプロンプト側です。`;` や `&&` によるコマンド連結は引き続き禁止しますが
  （許可リストはコマンド名の prefix で照合するため）、**それはシェル上の連結を禁じる規則で
  あって「1 ターンあたり Bash 呼び出し 1 回」の制限ではない**ことを明示しました。
- **実行履歴** (#643): 試行ごとに JSON オブジェクト 1 行を
  `${XDG_STATE_HOME:-~/.local/state}/knowledge-distill-radar/runs.jsonl`
  へ追記します（ディレクトリ 0700 / ファイル 0600、直近 200 件に切り詰め）。記録するのは
  ISO 週・機械可読な `status`・終了コード・試行回数・所要秒、そして envelope が残っていれば
  `num_turns` とその実行自身の `subtype` です。`num_turns` は、上記の上限値を決めるために
  セッショントランスクリプトを発掘して初めて得られた数値でした。今後は記録に残ります。
  `status` は `ok` / `max_turns` / `timeout` / `api_error` / `exec_error` / `no_report` /
  `no_claude` のいずれかで、`decided_by` がどのシグナルでそれを決めたかを示します。一次
  シグナルは終了コードと claude の stderr で、いずれも 4 件の失敗すべてで本番ログに直接
  観測されています。`--output-format json` は付加情報としてのみ読みます —— envelope は
  `type` フィールドを持たず、watchdog に殺された実行では envelope 自体が残らないためです
  （#526）。決め手となったシグナルを明示するのは、frontier-harness で同じ曖昧さに対して
  #526 が確立した作法に倣ったものです（envelope と終了コードは常に一致するとは限らず、
  両者を潰すと自分の status と矛盾する理由文になります）。ライブラリは
  `home/dot_claude/job-runlog.sh` → `~/.claude/job-runlog.sh` で、実行ではなく source される
  ため `executable_` プレフィックスを付けず 0644 で配備されます。意図的にジョブ非依存で、
  呼び出し側が自分の履歴ファイルを渡す形なので、morning-radar・macos-defaults-drift・
  #644 の「一定期間走っていない」検知は、このファイルにラベルを教えることなく再利用できます。
  現時点で配線しているのは knowledge-distill だけです。
- **再試行** (#643): `api_error` と分類された失敗のみ、60 秒後に 1 回だけ再試行します。
  それ以外は再試行しません。4 週連続の失敗のうち一過性だったのはちょうど 1 件 —— 08-07 は
  18:12:57（起床時の coalesce 発火）にネットワークが立ち上がる前の `ENOTFOUND` で死んで
  います。予算を使い切った実行を引き直しても、その週の予算を二重に使って同じ場所で死ぬ
  だけです。両方の試行が記録されます。
- **通知**: 成功時、`claude-attention` は priority 3（default）で `HEADLINE` を受け取り、
  precheck が dry pipeline と判定した場合は `[縮退]` と instinct 数のプレフィックスが
  付き、直前に記録された実行が失敗だった場合は `[復旧]` が付きます。エラー経路
  （claude 不在 / timeout / 非 0 exit / レポートファイル欠損）は priority 5 で publish
  されます（morning-radar のエラー時と同じ規約）。加えて、通知 1 本だけでは読み取れない
  2 点が付加されます（#643）: 同じ `status` が**異なる 2 週以上**で失敗している場合は
  `[N週連続/status]` を先頭に付け、最後の**成功**が 14 日より古い場合はその経過日数を
  末尾に付けます。連続数はレコード数ではなくスケジュール枠（週）で数えるため、再試行が
  同一週に 2 レコードを書いても「2 週連続」とは報告されません。#643 以前も個々の失敗は
  通知されていました —— 誰にも見えなかったのは「4 週続けて同じ倒れ方をしている」という
  ことです。実行履歴そのものが利用できないときは、通常週として通してしまわずその旨を
  通知に明記します。無言で欠落した記録は「一度も走らなかった週」と区別が付かず、これは
  #491 と同じ種類の故障です。

  **この仕組みが検知しないもの**: staleness の判定はジョブが走っている最中にしか読めない
  ため、答えられるのは「最近成功していない」であって「そもそも発火していない」ではありません。
  一度も発火しない LaunchAgent を検知するには外部の poller が必要で、それは #506 の
  スケジューリング作業の領分です。
