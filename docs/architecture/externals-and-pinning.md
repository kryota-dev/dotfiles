...
Critical policy: **every ECC bump is reviewed before it merges.** The ECC tarball contains executable hook scripts that run inside the agent harness, so `scripts/renovate-gate-classify.sh` names `affaan-m/ecc` in its always-review list: the update never reaches the deterministic fast lane and an agent reviews it, whatever the diff looks like. (Until the merge gate landed this was an `automerge: false` packageRule in `renovate.json5`. Config could only key on updateType, which cannot tell an ECC release apart from any other tagged one — see [Renovate automation](renovate-automation.md).)
