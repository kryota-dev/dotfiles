...
**通常パスのコスト。** statusline は毎ターン再描画されるため、ウィンドウ判定と「ディスク上に起点があるか」の探索はシェル組み込みだけで行います——quota を超過しない環境では追加のプロセス生成がゼロです。起点の書き込みは値が実際に動いたときのみで、終了済みセッションが残したファイルは 1 日 1 回バックグラウンドの `find`（mtime 7 日）で掃除されます。
