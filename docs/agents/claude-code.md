...
**Cost of the common path.** The statusline re-renders on every turn, so the window checks, the session-id resolution, and the "is there any baseline on disk" probe are shell builtins. The *billing* half of the file is still written only when a value actually moves (or when there is no prior file yet), so a machine that never exceeds its quota pays no extra billing write. Since #499, though, the `context` sibling key is written on every render whenever `context_window.remaining_percentage` is present — the same cost `write_harness_cost` and `write_rate_limits_snapshot` already pay on every render, extended to this file. A render that is inside quota (or has no `rate_limits` at all) therefore now does one small atomic write per turn to keep `context` live, where it previously did none. Files left behind by ended sessions are swept once a day by a backgrounded `find`; the sweep runs whatever this session's quota state is, since a machine sitting in a long overage would otherwise never collect them. (`find -mtime +N` compares whole 24-hour periods with a strict `>`, so effective retention is up to N+1 days.)
