...
## キュレーテッドスキルインベントリ（<!-- FACT:curated-skill-count -->51<!-- /FACT --> スキル）

`home/dot_agents/skills/` のキュレーテッドスキルをテーマ別にグループ化しています。

### Git、PR、GitHub ワークフロー（14 スキル）

日常的な開発サイクルの自動化の中核。2026-07-06 の棚卸しで並列開発トライアド（`repo-radar` → `issue-fleet` / `renovate-sweep`）が追加されました。`wave-orchestrator` はこの系統を「深く × 広く」へ広げ、独立セッションを並列に回して各々を `pr-workflow` の全長に通します。その `SKILL.md` は **norm registry**（守るべき規範と、そのまま使うコマンド形）で、規範の根拠となった実測・事故の経緯は `references/` にあります。
