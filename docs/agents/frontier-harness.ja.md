...
### 出力形状も契約であり、`--help` がそれを運ぶ

`fh <command> --help` は、そのコマンドの synopsis・受け付けるフラグ・そして **`--json` 出力が持つ
top-level キー**を出します。`--json` を足すと同じ契約が JSON で返るので、スクリプトは形状を推測せず、
依存する形をそのまま表明できます。
...
以前は `--help` が `flag-registry.mjs` の表から漏れており、`fh approvals --help` は
`unknown flag --help` を返していました。そのため `fh approvals --json` が配列ではなく
`{"approvals": [...], "skipped": [...]}` という envelope を返すことを、CLI から知る手段が
ありませんでした。監視スクリプトは top-level をその配列として舐め、jq が型エラーで落ち、
`2>/dev/null || true` がその失敗を空文字列に変え、空文字列は「承認要求 0 件」とも同じ見た目です。
承認要求 2 件が最大 42 分放置されました。直し方は表への追加であって、表への例外ではありません
——`--help` は表に載っているから通るのであり、`fh approvals --bogus --help` は今までどおり
`--bogus` を名指しして 64 で落ちます。

**契約は嘘になれません。** `cli.mjs` は `emit` を包み、そのコマンドの help が宣言していない
top-level キーが出たら内部エラーにします。payload は先に書きます——子セッションを報告する emit は
子が既に走ったあとに通るため、その記録を失うことは誤った help 文字列よりも悪い——そのうえで
未宣言のキーを名指しして 70 で終わります。実装にキーを足して文書化しなかった場合、それは全コマンド・
全分岐について、その分岐が最初に呼ばれた時点で落ちます。読み手が気付くのを待ちません。

逆向き——help が宣言しているキーを実装が出さなくなった場合——は
`tests/frontier_harness_cli_quality.test.mjs` が 2 層で覆います。同ファイルの
`OUTPUT_OBSERVED_HERE` に挙げたコマンドを実際に走らせ、**分岐ごとに**その分岐が宣言したキーが
そのまま出ることを要求します。`fh approvals` の一覧と `--purge`、`fh runs` の一覧と `--run` は
それぞれ別の envelope を返すため、コマンド単位で和集合を取ると片方のキーがもう片方に混ざっても
気付けません。安価に駆動できないコマンドについては、宣言キーが**それを出している module に**
残っていることを確かめます。ディレクトリ全体を検索すると、`status` / `command` / `taskId` の
ような名前は他所に残るだけで通ってしまい、実際にこの検査の初期版は `session-command.mjs` から
消えたキーを見逃しました。第 1 層からの除外は**理由を書いたときだけ**成立するので、実測対象を
減らすことは黙って起きず、レビューで見える行為になります。

残る未カバーは同ファイルに名指しで書いてあり、「駆動できないコマンドから落とされ、かつその名前が
同じ module 内に別の用途で残っているキー」だけです。

**`--help` がコマンドの代わりに割り込むことはありません。** `--json` と `--help` が効くかどうかは、
生の引数列ではなく**フラグ位置**で判定します。`assertKnownFlags` は値を取るフラグの次のトークンを
内容を見ずに読み飛ばすためです（`--timeout-ms -1` を通すための設計）。生の引数列を見ると
`fh approve --request <id> --deny --message "--help"` が help を出して exit 0 で終わり、**deny が
記録されないまま成功に見えます**——承認境界を閉じる操作が沈黙するということです。同じ理由で、
サブコマンドが解決できないとき（`fh session --bogus-flag --help`）は help を譲り、コマンド自身の
`fh session requires launch or resume, not --bogus-flag` を先に出します。そこは
`assertKnownFlags` が黙る唯一の条件であり、help で答えると打ち間違えたフラグが一言も出ません。
素の `fh session --help` は今までどおり help を返し、`fh approvals --bogus --help` は今までどおり
`--bogus` を名指しして 64 で落ちます。

`fh approve-server` は出力を空と宣言しています。これは stdio 上で MCP の permission-prompt
プロトコルを話し、自前の envelope を持たないためで、空であることは宣言し忘れではなく表明です
（全コマンドが「キー」か「持たない理由」のどちらかを宣言することをテストが要求します）。
...
**代わりに返るのが「その run が gate を通ったか」です。** 各 run は連結された決定的チェックの
件数 `verification` を持ちます（`total` と、`passed` / `failed` / `errored` の内訳）。`--run` では
その結果そのものもレコードと併せて返ります。`total: 0` の run は gate を 1 本も通していないので、
その `status: "succeeded"` が主張しているのは「ターンがエラーなく終わった」だけです。この区別は
以前は記録から読めませんでした —— 同じ wave の子 3 本が「CI の完了通知が来次第レビューへ進みます」と
述べて turn を終え、3 本とも `succeeded` / `exitCode: 0` として、実際に完走した子と外形が完全に
一致していました。`denials` も 3 本とも空です（拒否されたのではなく、やらなかったからです）。
判別できたのは PR のコメント数という外形からだけで、それはこの記録の一部ではありませんし、
今後もそうはなりません。
...
なぜ `PreToolUse` の `defer` hook や `PermissionRequest` hook ではなくこの方式なのか、
そして何があればその答えが変わるのかは
[ADR 0001](../decisions/0001-fh-approval-channel.ja.md) に記録してある。
...
`make` も承認可能なランナーですが、通るのは `make <target>` の形だけです —— 1 つ以上のターゲット名で、
各々が英数字か `_` で始まり、`[A-Za-z0-9_./-]` の範囲に収まるものに限ります。オプションと
command-line variable assignment は承認できないので、`make -f /tmp/evil.mk all` /
`make --file=… all` / `make -C /tmp/evil all` / `make SHELL=/tmp/evil test` はいずれも
「承認できる形ではない」として拒否されます。`make` 単体も拒否します。他のランナーが引数を必須に
しているのと同じ理由で、既定ターゲットに任せると承認した文字列が何を走らせるのか言わなくなるからです。

ターゲットに `..` を含めることもできません。上の字集合が縛るのは各ターゲットの**先頭 1 文字だけ**
なので、そうしないと `make a/../../etc/cron.d/evil` が通ってしまいます —— 「ターゲットは `.` で
始められない」という規則は、途中から遡る形を止めません。この検査は `make` にだけ掛かります。
`go test ./...` は既存の承認可能なコマンドで、その `..` は正当だからです。

字集合を狭めているのは意図的で、`make` にだけ適用します。上の段落が既に認めているとおり、承認済み
ランナーはリポジトリ側が握るものへ処理を委ねます —— `make test` が `Makefile` を読むのは
`npm run test` が `package.json` を読むのと同じことで、この間接性自体は新しくありません。
`-f` / `--file` / `-C` は種類が違います。これらは `make` を**リポジトリの外**の makefile へ向けるもので、
上の段落が拠って立つ唯一の前提を外します。`SHELL=` のような変数上書きも recipe の展開先を変えるので
同じ効き方をします。`npm run <script>` にはどちらに当たる引数も無いため、ここが `make` を
「違う形で」ではなく「より狭く」持つべき唯一の箇所です。

トークン化した完全一致はこれによって変わりません。`make lint` を承認するのはその文字列だけの承認で、
...
### 完了条件は宣言するもので、自称するものではない

`outcome` が言うのは「ターンがエラーなく終わったか」だけです。「指示されたことをやったか」は
一度も言っていませんし、この 2 つを分けたこと自体は設計意図でした（PRD 493 AC-005 / PRD 537
AC-012）。欠けていたのは**戻りの線**です —— 起動時の出力にも `adapter_runs` にも verification への
参照が無かったため、指示を飛ばした子と、こなした子が記録上まったく同じに見えていました。
...
`--gate` は完了条件を「承認済みの決定的チェック」として宣言します。複数あるなら繰り返します。
宣言の形は `<kind>:<command>`、または kind を省いた `<command>`（既定は `test`）です。接頭辞が
kind として読まれるのは閉じたチェック語彙に一致するときだけで、承認できるコマンドは必ず
タスクランナー名で始まるので衝突しません —— `npm run test -- --grep=a:b` はコマンドであって、
`npm run test -- --grep=a` という gate ではありません。

第 2 の機構を作らず `fh verify` の機構をそのまま使ったことから、次の 5 点が従います。

- **gate は子を起こす前に照合されます。** そのコマンドは capability と同じ manifest 照合を
  通るので、未承認の gate は gap を queue して exit 2 になります。子が数時間走ったあとで
  「その gate は承認されていない」と分かる形にはしません。
- **何で測られるかを子に伝えます。** 宣言したコマンドは、sandbox の制約を運ぶのと同じ
  固定語彙の説明として prompt の先頭に置かれます。子から見えない gate は条件ではなく罠です。
- **harness はやはり出力を見ません。** チェックは `check-runner.mjs` を通り、その stdout /
  stderr は端末へ継承されます。gate が記録へ足すのは終了コードだけで、自由文は足しません ——
  「決定的な検証」の節が述べる性質は、2 つ目のコマンドから到達しても弱まりません。
- **結果は run に連結されます。** 各チェックは `verification_results` の 1 行として記録され、
  その `adapter_run_id` がそのセッションの run を指します。この列は最初から存在していて、
  誰も書いていませんでした。書くようにしたことが、「この run は何かを通したのか」を記録だけから
  `fh runs` に答えさせています。
- **判定は下方向にしか動きません。** 落ちた gate は、子の envelope が成功を主張していても
  セッションを `failed` にします。そもそも起動できなかった gate は `indeterminate` にします ——
  `adapter_runs` はそれを理由付きの `failed` として保存するので、status の語彙は増えません。
  gate が失敗した子を成功に変えることはなく、確定した 1 本の赤が、走らなかった別のチェックに
  よって「判定不能」へ薄まることもありません。

成功しなかった子では gate を走らせません。gate が答える問い ——「ターンはきれいに終わったが、
通っているか」—— は、ターンがきれいに終わったときにしか立ちません。すでに失敗しているツリーで
走らせても、見落とされた条件ではなく先の失敗を説明する赤が増えるだけです。各チェックの上限は
`--gate-timeout-ms` で、これは「決定的な検証」の節が述べるチェックの制限時間そのものです
（既定も上限も同じ）。この値はチェックの直前ではなく**子を起こす前に**解決します —— 遅く読むと、
無効値の拒否が子の実行**後**に起き、`adapter_runs` の行が 1 つも残らないまま終わってしまい、
それはこのコマンドが最も避けようとしている失敗そのものだからです。`--timeout-ms` と別のフラグに
してあるのは、そちらが承認チャネルの待機時間だからで、1 つの名前に 2 つの制限は持たせられません。
gate の**本数**にも上限があります。個々に制限時間があっても、宣言できる本数が無制限なら
「本数 × 上限」で、その制限時間が閉じたはずの窓が開き直ります。

### gate が守ってくれないこと

gate は、子が走った sandbox の中では走りません。`check-runner.mjs` は承認済みコマンドを直接
起こすだけで、ネットワークもファイルシステムも自前の隔離を持ちません ——「決定的な検証」の節が
述べる性質そのままで、gate は第 2 の実行器を作らずその runner を再利用しているからです。
**新しいのは到達経路のほうです。** `fh verify` は人が打つコマンドですが、gate は harness が、
無人で、エージェントが同じツリーへ書き終えた直後に起こします。承認済みの文字列 `npm run test` が
指すのはその時点の `package.json` であり、それはまだ誰も読んでいない差分の一部です。

ここから 2 つのことが従い、どちらもコマンドを承認することでは閉じません。

- **gate のチェックは呼び出し元の環境を継承しません。** 届くのは固定の許可リストだけです ——
  `PATH` / `HOME`、一時ディレクトリとロケール、XDG のディレクトリ。それ以外は、harness 自身が
  起動時に持っていたトークンを含めてすべて落とします。禁止リストではなく許可リストにしたのは、
  禁止リストは次に現れる秘密の名前を取りこぼすからで、見えない変数を必要とするチェックは
  静かに通るのではなく大きな音を立てて失敗します。`fh verify` は従来のままです —— あちらの
  到達経路は今回変えたものではありません。
- **判定は依然として終了コードだけです。** gate が言うのは「承認済みコマンドを harness が
  走らせ、0 が返った」であって、「そのコマンドが何かを検証した」ではありません。自分のテスト
  スクリプトを書き換えた子は `verification.total > 0` かつ全 `passed` を出せますし、記録は
  それを本物の pass と区別できません。**できるのは、その書き換えを差分に残すことです** ——
  review packet は作業ツリー全体を staging するので、無害化されたテストスクリプトは gate からは
  見えなくてもレビュアーからは見えます。緑の gate は「harness がこれを走らせて 0 が返った」と
  読んでください。モデルがそう言っただけよりは厳密に強く、「変更が検証された」よりは厳密に
  弱い主張です。なお `fh verify --candidate` と違い、session の gate は tree hash を記録しません
  —— あれは candidate の取り込み判定のための仕組みで、その語義をここへ持ち込まないためです。

gate を宣言しなければ outcome は従来のままです。宣言の無い gate を `indeterminate` に落とすのが
「判定できないなら成功と言わない」の最も厳しい読み方ですが、それは既存の呼び出しを一斉に落とし
ますし、同じ事実はそれなしでも読めます —— 連結された検証結果が 1 件も無い run は何も通しておらず、
そこでの `succeeded` は「ターンが終わった」以上のことを意味しません。それを「作業が完了した」と
読むのは監視側が避けるべき誤りで、記録は PR のコメント数の代わりに読むべきものを持つようになりました。
