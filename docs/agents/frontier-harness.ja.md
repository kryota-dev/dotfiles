...
`tests/frontier_harness_cli_quality.test.mjs` が覆います。同ファイルの `OUTPUT_OBSERVED_HERE` に
挙げたコマンドを実際に走らせ、宣言キー集合と実測キー集合の一致を要求し、加えてどの宣言キーも
ソースから完全に消えていないことを確かめます。残る未カバーは同ファイルに名指しで書いてあり、
「安価に駆動できないコマンドから落とされ、かつ名前がソースの他所に生き残っているキー」だけです。
