...
1. zsh dmux ラッパーが dmux プロセスの `PATH` 先頭に `~/.config/dmux/bin` を追加します。
2. **dmux のエージェントペインは `~/.zshrc` を再 source する fresh な interactive zsh** であり、そこで `mise activate` が `PATH` を組み直してシム dir を脱落させます。そこで `~/.zshrc` は `mise activate` の**後で** `~/.config/dmux/bin` を再前置します（全 tmux ではなく `dmux-*` セッション限定。`tmux display-message -p '#{session_name}'` で判定）。これがペイン内でシム dir を mise 管理の実 `codex`/`claude` より前に保つ keystone で、これが無いとペインは実バイナリに直接解決しシムが走りません。dmux セッション限定にすることで、同一サーバの無関係な tmux ペインの素の `codex`/`claude` は影響を受けません（`--profile shared` なし、happy 誤起動なし）。
...
シム**と** `~/.zshrc` の再前置が無いと、dmux ペインは SSOT `shared.config.toml` なしで実 `codex` をサイレントに実行してしまいます。
...
`DMUX_HAPPY=1 dmux` で `happy claude` に切り替わります。トグルをペインに届けるには2点が必要です:

- **PATH**: 上記 `~/.zshrc` の再前置（codex シムと共通）が、ペイン内でシム dir を mise の実
  `claude` より前に保ちます。
- **Env**: tmux はペインの環境をサーバ起動時に固定するため、`dmux` / `dmux-r06` ラッパーが
  `tmux set-environment -g DMUX_HAPPY 1`（解除は `-u`）で稼働中サーバにトグルを注入します。
  これが無いと再利用サーバは後からの `DMUX_HAPPY=1 dmux` を認識しません。

シムは `exec happy claude` の前に実 claude を `HAPPY_CLAUDE_PATH`（happy が claude 探索で最優先
に見る）へ pin するため、happy はその絶対パスを直接 spawn し、シム dir が PATH 上にあっても
シムへ再突入しません — 再帰は構造的に不可能です。
