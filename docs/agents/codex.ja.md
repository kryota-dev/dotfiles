...
2. **dmux のエージェントペインは `~/.zshrc` を再 source する fresh な interactive zsh** であり、そこで `mise activate` が `PATH` を組み直してシム dir を脱落させます。そこで `~/.zshrc` は `mise activate` の**後で** `~/.config/dmux/bin` を再前置します（`[[ -n "$TMUX" ]]` ゲート）。これがペイン内でシム dir を mise 管理の実 `codex`/`claude` より前に保つ keystone で、これが無いとペインは実バイナリに直接解決しシムが走りません。
