...
ベア呼び出しではないもう 1 つの leak 経路があります: `codex@openai-codex` プラグインの **`codex-rescue` エージェントは `CODEX_HOME` を伝播しない**ため、業務アカウントのセッションから起動すると Codex が個人アカウントに対して動きます。そのため skill はこれを一切起動しません —— 禁止の全文と他の理由は [Codex CLI ハーネス](codex.ja.md) を参照してください。
