...
The profile preserves the workspace-write filesystem boundary while allowing the main session to read GitHub state and perform live web research. Network access does not authorize repository or GitHub writes: worktree creation, commit, push, pull-request draft preparation and creation, and ready-for-review remain fixed `agent-workflow` host actions, each requiring Codex's one-time native command approval. Worktree creation accepts only a clean `main` worktree and creates a fixed sibling path through `git worktree add` with repository hooks disabled; it never invokes `wtp`, repository-defined hooks, or `direnv allow` with host privilege. The runner re-execs in a minimal fixed environment before invoking Git or GitHub CLI, and an action for an existing run must originate from that run's canonical linked worktree. Before a host commit it invokes a pinned account-managed gitleaks binary with a fixed global configuration, fails closed if either is unavailable, and ignores worktree-local gitleaks configuration; worktree creation, staging, commit, and push all disable repository and local hooks so a worktree cannot run code with host privilege. It is not used by non-interactive children; those use `agent` or `shared` explicitly.
