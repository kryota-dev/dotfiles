...
**Decision:** ECC's `pre:edit-write:gateguard-fact-force` gate (state impact facts before the first edit of each file) is off, and the long-session frictions catalogued in #282 are not escalated upstream. #282 was closed as moot; no upstream issue was filed. It was disabled via `env.ECC_DISABLED_HOOKS` from #280; **#496 went further and deleted the `PreToolUse` entry itself**, on the grounds that wiring nobody intends to run is dead config that reads like an active safety decision (#473 AC-011).
...
Upstream independently fixed part of this after the v2.0.0 pin (status as of 2026-07): condensed denials after a per-session budget (affaan-m/ECC#2227) and opt-in path exemptions via `GATEGUARD_EXEMPT_GLOBS` (affaan-m/ECC#2432). The remaining frictions (TTL, read-before-edit stacking, deny-message honesty) only matter if the gate is re-enabled. Re-enabling now means **adding the `PreToolUse` entry back to `settings.json`** — since #496 there is no entry to un-disable, so the `claude --settings` overlay question from #281 no longer applies to this gate. Filing an upstream issue for a gate this repo keeps off would create a tracking obligation with no local benefit.
...
See [claude-code.md](../agents/claude-code.md) for the hook graph (which no longer lists this gate) and the `ECC_DISABLED_HOOKS` / `ECC_DISABLED_HOOKS_EXTRA` channels. `tests/claude_hooks.bats` asserts the id stays absent from both the hook surface and `ECC_DISABLED_HOOKS`.
