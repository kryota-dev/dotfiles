...
名前の付いた代替案を比較したうえで下した判断のうち、却下した選択肢とその判断を覆す条件を残す価値があるものは、ここに節を足すのではなく [`docs/decisions/`](../README.ja.md#決定記録adr) の日付付き記録として書きます。
...
**判断:** ECC の `pre:edit-write:gateguard-fact-force` gate（各ファイルの初回編集前に影響ファクトの明示を要求）は無効とし、#282 で整理した長時間セッションでの摩擦は上流へエスカレートしません。#282 は moot としてクローズし、upstream issue は起票しませんでした。#280 では `settings.json` の `env.ECC_DISABLED_HOOKS` による無効化でしたが、**#496 で `PreToolUse` エントリ自体を削除しました** —— 誰も動かすつもりのない配線は、有効な安全判断のように読めるだけの死んだ設定だからです（#473 AC-011）。
...
上流は v2.0.0 ピンの後にこの一部を独立に修正しています（2026-07 時点の状況）: セッションごとの予算超過後の denial 圧縮（affaan-m/ECC#2227）と、`GATEGUARD_EXEMPT_GLOBS` によるオプトインのパス除外（affaan-m/ECC#2432）。残る摩擦（TTL、read-before-edit との重なり、deny メッセージの誠実さ）は gate を再有効化した場合にのみ問題になります。再有効化とは、いまや **`settings.json` へ `PreToolUse` エントリを追加し直すこと**を意味します —— #496 以降は「無効化を解除する」対象のエントリが存在しないため、#281 の `claude --settings` オーバーレイの論点はこの gate には当てはまりません。この repo が無効のまま維持する gate のために upstream issue を起票しても、ローカルの利点がない追跡義務が生まれるだけです。
...
フックグラフ（この gate はもう載っていません）と `ECC_DISABLED_HOOKS` / `ECC_DISABLED_HOOKS_EXTRA` チャンネルについては [claude-code.ja.md](../agents/claude-code.ja.md) を参照してください。この id が hook 面と `ECC_DISABLED_HOOKS` の両方から消えたままであることは `tests/claude_hooks.bats` が検証します。

---

## デュアルアカウントモデル: 設定は共有、状態は分離

**判断:** r06 作業アカウント（`~/.claude-r06`）は、すべての設定ファイル（settings、statusline、agents、commands、skills、CLAUDE.md）を `~/.claude` へ指す 6 つのシンボリックリンクとして実装されています。実行時の状態は、`claude`/`codex` ランチャーラッパースクリプトが設定するアカウントごとの環境変数によって分岐します。
...
このモデルのリスクは、#345 より前は 2 か所（`claude.zsh` の `_claude_with_home` と `codex.zsh` の `cdx-r06`）でアカウントごとの env セットが手動で定義されることでした——当時は受け入れられた重複で、代替案（両方から呼ばれる共有 env 構築関数）は変更が稀なセットに不必要な間接性を追加するだけだったためです。#345 はこの重複を除去する形で解決しました：各ハーネスの env セットは、その wrapper スクリプト（`~/.local/launchers/claude`、`~/.local/launchers/codex`）で 1 箇所だけ定義され、どの短縮名からもベアのバイナリ名からも同一に到達します。

アカウントごとの全 env 変数と完全なランチャーコマンドマトリクスについては [account-isolation.ja.md](../agents/account-isolation.ja.md) を参照してください。

---

## シークレットはソース（export なし）、サブプロセスにスコープして export

**判断:** 1Password レンダリングのキーファイル（`~/.config/zsh/claude-secrets.zsh`）は `export` なしでソースされます。`claude` ランチャーラッパーはこれを自分自身の短命なプロセス内で（インタラクティブシェルではなく）ソースし、これから `exec` する特定の呼び出しにスコープしてキーをインラインで export します。

**なぜ:** インタラクティブシェルがソースするファイル内の `export` は、セッションの存続期間中、すべての子プロセス——すべてのサブシェル、すべての外部コマンド、すべてのバックグラウンドジョブ——にキーを漏洩させます。不正プロセスや誤った `env` ログがプロセス環境をキャプチャすれば、キーが露出します。

export なしでのソースは、変数をローカルスコープに保持し、子プロセスには伝播しません。サブプロセスにスコープした export（ラッパーが本物のバイナリを `exec` する直前の `export EXA_API_KEY="${EXA_API_KEY:-}"`）により、Claude Code が必要とする正確な場所（MCP サーバーの env プレースホルダーを解決する場所）でのみキーが使用可能となり、それ以外では使用できません——ラッパープロセス自体は `exec` で置き換わるだけで、親として残り続けることはありません。

各 export の `${VAR:-}` デフォルトは、ランタイムの graceful degradation パスです。マシンでまだ `chezmoi apply` が実行されていない場合（シークレットファイルが存在しない場合）、ラッパーがエラーになる代わりに MCP サーバーはキーなしで起動します。
