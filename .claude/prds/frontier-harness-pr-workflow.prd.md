...
- AC-032: `/sdd` は requirements/design/task decomposition を生成する spec compiler へ縮退する。worktree、worker selection、implementation、commit、PR 作成を所有しない。 **（#503 で部分達成）** SKILL.md は spec compiler へ縮退し、上記の各契約は所有 skill を SSOT として参照する形になったが、`sdd` は依然 phase 4〜6 の実行主体である。実行主体の移管は #347 の single-approval 決定と衝突するため ADR を要し、#585 が扱う。
