...
- 2026-09-02: #503 で skill graph 移行を完了。(1) `multi-review` を reviewer roster と finding schema を持つ registry へ縮退し、Phase 1〜5 の実行手順・Codex leg 起動形・投稿手順・事実確認・エラーハンドリングを `references/` へ移設（SKILL.md 84,041 B → 31,036 B。規範の削除はゼロで、移設先は SKILL.md からの Read 指示付きで参照する）。(2) `sdd` を Phase 0〜3 の spec compiler へ縮退し、Phase 4〜6 を `references/` へ移設して worker 委譲契約は `pr-workflow` / `codex`、commit・PR 規約は `commit` / `create-pr` を SSOT として参照する形にした（AC-029 は達成。**AC-032 は部分達成** —— 規約の所有権は移したが、`sdd` は依然 implementation / commit / PR 作成の**実行主体**である。`pr-workflow` の standard/large path が「`/sdd` が commit + PR まで自律実行する」ことを前提に承認点を 1 つに絞っている（#347）ため、実行主体の移管は `pr-workflow` 側の設計変更を伴う。#585 へ送る）。(3) AC-034 の compatibility shim を撤去（撤去条件の実測は PRD の Open Questions 参照）。§4 contract と over-provision gate は残置し、`tests/files.bats` に floor 生存テストを追加した（floor 行の削除・非 blocking 化・caller の gate 呼び出し削除・shim 再導入の 7 ミューテーションで fail することを確認済み）。(4) 改修 3 skill に AC-040 の before/after eval を追加し、`tests/skill_provenance.bats` に references リンク解決と evals.json 形状の検証を追加した。
- 2026-09-02: #585 への申し送り。(a) AC-032 の実行主体移管（上記の部分達成の残り）。移管するなら #347 のsingle-approval 決定と衝突するため ADR が要る。(b) `pr-workflow/SKILL.md` L23 の「移行期間中」「shadow rollout」という表現が #503 で stale 化した（rollout は `pilot`、AC-034 shim は撤去済み）。(c) AC-040 の `pr-workflow` 用before/after eval は未着手のまま。
- 2026-09-02: #610 への申し送り。`wave-orchestrator` 自身の（子ではなく親の）セッションは `/model-fitness-check` をスラッシュ起動しておらず、floor 判定は設計上つねに子の `pr-workflow` 経由でしか掛からない。shim 撤去前後で変化しない既存事項だが、同 skill を縮小する際に orchestrator 自身の tier に floor を適用すべきかを明示的に扱うこと。
